...
var app = {
    srcPath: 'src_public',
    buildPath: 'build_public',
    distPath: 'public',
    no_srcPath: '!src_public',
    no_buildPath: '!build_public',
    no_distPath: '!public',
};
...
gulp.task('script',function(){
  gulp.src([app.srcPath+'/js/**/*.js',app.no_srcPath+'/js/**/*.min.js',app.no_srcPath+'/js/**/jquery.downCount.js',app.no_srcPath+'/js/**/*0.js',app.buildPath+'/concat_js'])
    .pipe(plumber({errorHandler: notify.onError('Error: <%= error.message %>')}))
    //当编译时出现语法错误或者其他异常,出现异常并不终止watch事件（gulp-plumber），并提示我们出现了错误（gulp-notify）。
    .pipe(jshint()) //检查JS错误
...
gulp.task('style',function(){
  gulp.src([app.srcPath+'/css/**/style.css',app.no_srcPath+'/css/**/*.min.css',app.no_srcPath+'/css/**/*0.css',app.no_srcPath+'/css/**/default.css',app.no_srcPath+'/css/**/iconfont.css'])
    .pipe(plumber())
    //.pipe(plumber({errorHandler: notify.onError('Error: <%= error.message %>')}))
    //当编译时出现语法错误或者其他异常,出现异常并不终止watch事件（gulp-plumber），并提示我们出现了错误（gulp-notify）。
    .pipe(autoprefixer({
...
//优化图片
  gulp.src([app.srcPath+'/images/public/**/*.{png,jpg,gif,ico}',app.no_srcPath+'/images/public/**/*_0.{png,jpg,gif,ico}'])
    .pipe(gulp.dest(app.buildPath+'/images/public'))
    .pipe(cache(imagemin({
    // cache( 减少压缩图片的内存消耗
    ...
    .pipe(gulp.dest(app.distPath+'/images/public'))
    .pipe(browserSync.reload({stream:true}));
    //执行无需F5自动刷新页面
});
/*img图片压缩结束*/
...
gulp.task('serve', function() {
    ...
    gulp.watch(app.srcPath+'/images/public/**/*', ['imageMin']);
